...
class HybridRetriever:
    def __init__(self, vectordb: Chroma, docs):
        ...
        self.docs = docs or []
        # Prepare BM25 tokenization carefully: guard empty docs
        tokenized = [ (d.page_content or "").split() for d in self.docs ]
        # BM25 requires non-empty token lists; if empty, replace with ['']
        tokenized = [t if len(t) > 0 else [""] for t in tokenized]
        self.bm25 = BM25Okapi(tokenized)
        # store embeddings if you want to rerank with vector embeddings later
    ...
    def retrieve(self, query, top_k=5):
        ...
        """
        # 1) vector similarity search (may return Document objects)
        try:
            vector_results = self.vectordb.similarity_search(query, k=top_k)
        except TypeError:
            # some wrappers use different param name
            vector_results = self.vectordb.similarity_search(query, k=top_k)

        # 2) BM25 scores (ranking over provided docs)
        ...
        # 3) Combine & re-rank: vector votes weighted higher
        combined = {}
        for i, d in enumerate(vector_results):
            ...
            combined[key]["score"] += 2.0  # vector higher weight
        ...
        for i, d in enumerate(bm25_results):
            ...
            key = f"{src}::{hash(text) % (10**8)}"
            combined.setdefault(key, {"doc": d, "score": 0.0})
            combined[key]["score"] += 1.0  # bm25 vote

        # Convert to sorted list
        ranked = sorted(combined.values(), key=lambda x: x["score"], reverse=True)

        # Build output list of top_k entries
        output = []
